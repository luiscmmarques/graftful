...
The cost is real. Acquisition is still measurable from Cloudflare's edge HTTP traffic, since a flyer landing on `/cto` is a real document request — but **engagement is not**. The service worker answers later navigations from cache and screen changes are client-side routing, so an offline-first PWA is invisible at the edge after the first visit. Nobody will know whether people who arrive reach the Stock screen.

Revisited September 2026: a decision now does wait on usage evidence — the app costs money to keep alive, and keep-or-retire needs to know whether anyone arrives. The staged answer is in `TODO.md` § 4: edge HTTP traffic and distinct acquisition paths first, which change nothing about the privacy claims; the first-party counter only if retention becomes the question, and only together with updated privacy copy in all five languages. The third-party beacon is no longer the cheap fallback it once was — its CSP allowances were removed along with the plumbing, so restoring it means widening a policy that is now `'self'` and nothing else.
