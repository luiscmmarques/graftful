...
- [x] **Launch languages: English and French**, with German, Portuguese and Italian shipped for everything that leaves the app. French is the launch language and first audience; English is the _type_ source in `src/lib/i18n/en-source.ts`, which is a build-time role rather than an editorial one — see `STACK.md`.
- [x] **Tips: a plain PayPal link.** A plain hyperlink only — never PayPal's embedded button, which loads their JavaScript and would break the "no third-party requests" property. No other payment method: anything that encodes personal contact details on a health-condition project is a bigger disclosure than a name.
...
Working and testable: `npm run dev`. 156 unit tests, 16 app and 3 offline end-to-end tests, 0 type errors, offline precache generated and verified with the origin killed.
...
- [x] i18n system: typed catalogue, browser detection, Setup override, 5 languages
- [x] Order text and calendar export in English, French, German, Portuguese, Italian
...
**Done, and complete in all five languages — the four-language migration finished on 3 September 2026, and Italian followed:**
...
Deliberately no runtime fallback: a key enters the catalogue only once all five languages exist. An untranslated screen is therefore visibly, consistently English rather than a patchwork, and `npm run check` can never go green over a half-finished language.
...
- [x] **PayPal link live** — `PAYPAL_URL` set to `paypal.me/LuisMarquesCH` in `src/routes/support/+page.svelte`; the donate card shows in all five languages with the personal note. The name disclosure was decided knowingly: it already appears in `LICENSE` and the About page.
...
Verified September 2026, after finding the code claimed one model and implemented another. The real path: a deploy publishes a new `service-worker.js` (served `no-cache`, so never stale at the edge). The browser revalidates it at every cold launch, and `registerServiceWorker` additionally checks on every return to the app (throttled to hourly) — without that resume check, a phone PWA that is only ever resumed could stay pinned to an old version indefinitely. The new worker installs and precaches the new version in the background, then **waits**: it takes over on the next launch, once every instance has closed. Meanwhile the old worker serves entirely from its own precache, so the outgoing version keeps working rather than 404ing.

Deliberately no `skipWaiting` at module scope and no update prompt. Swapping workers under a running page breaks it — the page holds the chunk set it booted with, and `cleanupOutdatedCaches` deletes those chunks on activation, so the next lazily loaded route fails mid-session. One launch of lag is the safe trade for an app someone may be mid-way through a stock recount in. The `SKIP_WAITING` message listener stays in the worker as the escape hatch a future update prompt would use; sending it must be paired with a reload on `controllerchange`.
...
- [x] **Home / Today** — what to take at each slot, per product
...
- [x] **Privacy note** — shipped at `/privacy`: local-only storage, nothing collected, how to verify it. Revised Swiss FADP in force since Sept 2023; health data is sensitive, so it is explicit and readable. Needs a line about the push server when v2 lands.
- [x] **Intended use / disclaimer** — shipped as the intended-use statement on `/about` (`purposeStatement`); the same wording feeds the Swissmedic enquiry (§8)
- [x] **Support / tip page** — shipped at `/support`, with the donate card live (a plain PayPal link — see § 0 for the tips decision)
- [ ] **FAQ** — including "what happens if I clear my browser data"
- [x] **Contact** — shipped as the contact section on `/support` (email, GitHub forms, security address)
...
Five catalogues exist and are complete for every screen, the content pages, the pharmacy order text and the calendar export — the last two matter most, being the only things that leave the device. What is deliberately left in English, and why, is recorded once under **Language: what is and is not done** in § 0b rather than repeated here.
...
- [x] Italian (standard, not a Ticino variant)
...
- [ ] Distinct path per location (see § 4 — paths, not `?src=`, are what edge traffic can count)
- [ ] The local GP — a warmer channel than a poster
...
- [x] **The service worker was served `max-age=14400` on the live domain**, not the `no-cache` that `_headers` asks for — **fixed 4 September 2026** by setting **Caching → Configuration → Browser Cache TTL → `Respect Existing Headers`** on the zone. That was the diagnosis all along: files get Cloudflare Pages' own `public, max-age=14400, must-revalidate` when nothing overrides it, and the zone-level Browser Cache TTL was overriding `_headers` for JavaScript responses specifically — every document and even `manifest.webmanifest` had always arrived correctly, which is what made it look like a Pages bug rather than a zone setting.

  Verified against production after the change, including the three responses that had to _not_ move, since a zone-wide cache setting is exactly the kind of fix that breaks something else: `/service-worker.js` → `no-cache`; a hashed asset → `public, max-age=31536000, immutable`; `/` → `no-cache, no-transform`; `manifest.webmanifest` → `no-cache`. The immutable row was the real risk, and it survived because `_headers` states every rule explicitly, so Cloudflare had nothing to substitute a default for.

  Worth keeping the reason this was never urgent: the app registers without `updateViaCache`, so it defaults to `imports` and browsers bypass the HTTP cache when checking a worker for updates. The point of fixing it is that relying on a browser default to cancel out a server misconfiguration is not a property to depend on.
...
Worth checking the employer's policy on personal open-source work before publishing too, since that is a separate question from the licence.

- [x] Set the per-repo commit identity above — repo-scoped, global config untouched. Now uses the GitHub noreply address (`luiscmmarques@users.noreply.github.com`), so future commits publish no personal mailbox; enable "Block command line pushes that expose my email" in GitHub → Settings → Emails to enforce it.
...
### Clinic-authored regimen files (v2)

Document the backup JSON schema publicly, so a hospital, transplant centre or pharmacy can generate a regimen file for a patient to import — replacing the most fragile step of onboarding (a newly transplanted person typing eleven products by hand, three weeks post-surgery) with one file and one import.

What it takes:

- A versioned, human-readable schema page describing the export format (`EXPORT_VERSION`, every field, the units-are-primary rule, worked example with invented names). The format already exists and already validates on import (`parseImport` rejects malformed files with named warnings), so this is documentation plus a stability promise, not new code.
- A commitment about compatibility: the schema page states which `EXPORT_VERSION` it describes, and imports keep refusing files from a _newer_ version rather than half-reading them.
- Possibly a minimal generator page or spreadsheet template later — but the schema document alone is enough for a hospital IT department or a pharmacy system vendor.

The regulatory position stays clean and is worth stating on the schema page itself: the file is a transcription of a prescription, authored by the professional who holds it. Graftful still computes nothing clinical — it imports what the clinician wrote exactly as it imports what the patient typed. The one thing to be careful about: the schema documentation must not ship default doses, example strengths tied to real products, or anything that looks like a starter regimen — the worked example uses the same invented names as the seed.
