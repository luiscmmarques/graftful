...
There is deliberately no runtime fallback. A key enters the catalogue only once all four languages have it, so a half-translated screen is visibly and consistently English rather than a patchwork — and `npm run check` can never go green over a half-finished language. Order, Setup and the content pages are still English; the app shell, Today, Stock, the pharmacy order text and the calendar export are translated into all four.
