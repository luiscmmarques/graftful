...
/*
 * `clientsClaim` matters most on first visit. People arrive by scanning a QR code on
 * a flyer in a hospital corridor; without claiming, the first load leaves the page
 * uncontrolled, and losing signal before navigating again means nothing was cached.
 *
 * There is deliberately no `skipWaiting` at module scope. An updated worker installs
 * and precaches the new version in the background, then waits; it takes over on the
 * next launch, once every instance of the app has closed. Swapping workers under a
 * running page would be the bug, not the feature: the page holds the chunk set it
 * booted with, `cleanupOutdatedCaches` deletes those chunks on activation, and the
 * next lazily loaded route would then fail mid-session. The old worker meanwhile
 * serves entirely from its own precache, so the outgoing version keeps working — one
 * launch of lag is the safe trade. The update check itself runs at every cold launch
 * and on every return to the app: see src/lib/registerServiceWorker.ts.
 */
clientsClaim();
...
/*
 * Escape hatch for applying a waiting update immediately. Nothing in the app sends
 * this today — updates apply on the next launch by design, see the note above — but
 * the listener is the standard Workbox convention, and it is what an update prompt
 * would use if one is ever added. Sending it must be paired with a reload on
 * `controllerchange`, or the page outlives its own chunk set.
 */
