...
/**
 * Where a donation goes. Rendered only through `src/lib/DonateLink.svelte` — a plain
 * link, never PayPal's embedded button, which loads their JavaScript and would break
 * the "no third-party requests" property the privacy page depends on. A paypal.me
 * page shows the account holder's name; that disclosure was made knowingly (the name
 * is already in LICENSE and on the About page).
 */
export const donateUrl = 'https://paypal.me/LuisMarquesCH';
