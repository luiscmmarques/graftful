 ...
 *
 * Migration is progressive and honest: keys live here only once all five languages have
...
export interface Messages {
	...
	about: {
		...
		madeByAfter: string;
		madeByNote: string;
		/** The donate sentence in "Who made it". `donateAfter` carries its own leading punctuation. */
		donateBefore: string;
		donateLink: string;
		donateAfter: string;
		licenceTitle: string;
		...
		licenceName: string;
		/**
		 * Third-party attribution, covering every brand the app names anywhere: PayPal on
		 * Support, GitHub on About/Roadmap/Support, Cloudflare on Privacy, Kiro here.
		 * One app-wide note on the colophon rather than boilerplate on every page.
		 */
		trademarks: string;
