...
const STRINGS = {
	...
	it: {
		subject: 'Ordine di farmaci',
		greeting: 'Buongiorno,',
		request: (note?: string) =>
			note
				? `Vorrei ordinare i seguenti farmaci, se possibile per ${note}:`
				: 'Vorrei ordinare i seguenti farmaci:',
		box: (n: number) => (n === 1 ? 'scatola' : 'scatole'),
		// Invariant in Italian, so one form covers both.
		unit: () => 'unità',
		outro: 'Grazie in anticipo e cordiali saluti.'
	}
} as const;
